%% Behavioral load and process

behavior = readtable("Behavior.csv");

% Initialize a logical index of rows to remove
remove_row = (behavior.RECORDING_SESSION_LABEL == 18 & ...
              (behavior.article_id_pos == 15377 | behavior.article_id_pos == 342)) | ...
             (behavior.RECORDING_SESSION_LABEL == 31 & ...
              (behavior.article_id_pos == 104 | behavior.article_id_pos == 13388 | ...
               behavior.article_id_pos == 15629 | behavior.article_id_pos == 17021 | ...
               behavior.article_id_pos == 457 | behavior.article_id_pos == 47 | ...
               behavior.article_id_pos == 53 | behavior.article_id_pos == 7331 | ...
               behavior.article_id_pos == 9188));

% Remove the rows
behavior(remove_row, :) = [];
behavior.ValenceDiff = behavior.VARIABLE_rating_post_valence - behavior.VARIABLE_rating_pre_valence;

groupsummary(behavior, ...
    {'RECORDING_SESSION_LABEL', 'VARIABLE_music_type'}, ...
    'mean', {'VARIABLE_rating_pre_arousal','VARIABLE_rating_pre_valence', ...
             'VARIABLE_rating_post_arousal','VARIABLE_rating_post_valence','ValenceDiff'});


groupedResults = groupsummary(behavior, ...
    {'RECORDING_SESSION_LABEL', 'VARIABLE_music_type', 'type_pos'}, ...
    'mean', { ...
      'VARIABLE_CORRECT', ...
      'VARIABLE_READING_TIME', ...
      'VARIABLE_rating_pre_arousal', ...
      'VARIABLE_rating_pre_valence', ...
      'VARIABLE_rating_post_arousal', ...
      'VARIABLE_rating_post_valence', ...
      'ValenceDiff' ... 
    });

groupedResults.Properties.VariableNames = { ...
    'RecordingSession', 'MusicType', 'PassageType', ...
    'GroupCount', 'Accuracy', 'AvgReadingTime', ...
    'AvgPre_arousal', 'AvgPre_valence', 'AvgPost_arousal', 'AvgPost_valence', ...
    'Mean_Valence_diff' ...
};

happy_global = groupedResults(strcmp(groupedResults.MusicType, 'Happy') & strcmp(groupedResults.PassageType, 'global'), :);
sad_global   = groupedResults(strcmp(groupedResults.MusicType, 'Sad')   & strcmp(groupedResults.PassageType, 'global'), :);
happy_local  = groupedResults(strcmp(groupedResults.MusicType, 'Happy') & strcmp(groupedResults.PassageType, 'local'),  :);
sad_local    = groupedResults(strcmp(groupedResults.MusicType, 'Sad')   & strcmp(groupedResults.PassageType, 'local'),  :);

orderedResults = [happy_global; sad_global; happy_local; sad_local];
%% get significant
addpath(genpath('src\'))
